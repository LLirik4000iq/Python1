...
class Dog:
    rand = {
        'Corm': {'money_spend': 30, 'gladness_up': 50},
        'Park': {'money_spend': 10, 'gladness_up': 25},
    }

    def __init__(self):
        self.activity = random.choice(list(Dog.rand))
        self.money_spend = Dog.rand[self.activity]['money_spend']
        self.gladness_up = Dog.rand[self.activity]['gladness_up']
...
class Human:
    ...
    def is_alive(self):
        if self.gladness < 0:
            ...
            print('Bankrupt...')
            return False
        death = random.randint(1, 300)
        if death == 7:
            print('Dog dead. Depression...')
            return False
        else:
            return True
